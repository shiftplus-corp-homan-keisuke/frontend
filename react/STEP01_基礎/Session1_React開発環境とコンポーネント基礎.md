...
学習の過程で、これらの基本スキルを使って zevo のプロンプト例カードも作成し、React の実践練習も行います。
...
まず不要なファイルを削除し、プロジェクトを一から作成します。ブラウザを右、エディタを左に配置して、常に出力を確認できるようにしましょう。
...
そこで最初から、エラーや問題への対処テクニックを紹介します。これは重要なので必ず読んでください。
...
このコースで重要なのは、デザインをコンポーネントに分解する力です。
...
スターターフォルダから必要なファイル（pizzas 画像・data.js・index.css）を public フォルダにコピーします。
...
ピザに関するデータを含むので、これを Pizza と呼んでいます。今のところ、この関数にはパラメータはなく、関数本体があります。
...
**第二に、関数は何らかのマークアップを返す必要があります**。通常は JSX の形で、しかし何も返さない、つまり null を返すことさえできます。

しかし、ここでは、例えば H2 要素を返して、「pizza」と書きましょう。

保存しても、もちろんユーザーインターフェースには何も表示されません。それは、この新しいコンポーネントをどこにも含めていないからです。ESLint もここで警告しています。

この黄色い線は「pizza が定義されているが、使用されていない」と言っています。
...
Pizza と書いて、すぐに要素を閉じます。

しかし、これはエラーを発生させます。以前にも見たことがあるエラーです。このエラーの理由は、各コンポーネントは正確に 1 つの要素のみを返すことができ、ここにあるような 2 つの要素は返せないからです。

そこで、これを div でラップしましょう。VS Code が自動的にこれらの要素を閉じるのが少し煩わしくなることがありますが、とにかく、今度は UI にピザが表示されます。
...
さて、この pizza コンポーネントをもう少し興味深いものにしましょう。そのために、ここのスターターデータを使用します。
