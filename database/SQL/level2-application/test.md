...
### 問題3
次のCASE文の実行結果として正しいものはどれか。（salary = 7500の場合）
...
A) `WHERE salary > 5000`のクエリで効果的に使用される  
B) `WHERE department_id = 10`のクエリで効果的に使用される  
C) `WHERE salary > 5000 AND department_id = 10`のクエリでは使用されない  
D) ORDER BY句では使用できない
...
#### 問題3 - 解答: B
CASE文は上から順に条件を評価します。salary = 7500の場合、最初の条件（>= 10000）は偽、2番目の条件（>= 5000）は真となるため、'中給'が返されます。
