...
さて、これまでのアプリケーションに新しい機能を追加してみましょう。現在は単一の状態（ステップ番号）のみを管理していますが、実際のアプリケーションでは複数の状態を同時に管理することが一般的です。
...
さて、ここで変更できる小さなことが1つあります。そして、これは状態とは関係ありません。それは、実際には1つの要素だけを返す必要はないということです。

基本的に両方、この開閉ボタンとステップコンテンツを返したいのです。そして、したがって、これはReact Fragmentの優れたユースケースです。

まず、検査してみましょう。確かに、ルートがあります。それは基本的にすべてです。アプリ全体です。そして、その中に、このdivがあります。

しかし、たとえば、それを望まない場合。ボタンだけでなく、そして、このステップdivも同じレベルに配置したい場合。

そこで、不要なラッパー要素を削除して、フラグメントを使用しましょう。フラグメントはこのJSX要素のルートのようなものです。これはDOMでは消えます。
...
ウェブ開発者として、私たちは開発者ツールに大きく依存しています。ブラウザのコンソールや要素の検査パネルのようなものです。ツールは開発者にとって非常に役立つので、ReactチームはReact専用の開発者ツールを構築しました。

これは、状態を扱う場合に非常に役立ちます。今状態を扱っているので、それらを確認しましょう。

### インストール方法

実際、最初から、コンソールにこのメッセージが表示され、これらの開発者ツールをダウンロードするように指示しています。コンソールを開くことができます。そして、ここでReactドキュメントの場所へのリンクが見つかります。そこで、開発者ツールへのリンクを見つけることができます。

Chrome Web Storeへのリンクはこちらです。そして、他のブラウザ用もあります。しかし、Google Chromeを使用しているので、これが私たちが必要なものです。

何らかの理由でこのリンクがコンソールに表示されなかった場合は、「chrome react dev tools」とGoogle検索するだけです。はい、これは最初のものです。

このページにアクセスしたら、この拡張機能をダウンロードして、Google Chromeにインストールできます。

### 開発者ツールの使用方法

ツールをインストールしたことを願っています。他のChrome拡張機能のようにインストールします。一度完了したら、開発者ツールがすでに開いている場合、React開発者ツールから来るタブが1つ、または実際には2つが表示されます。

これはComponentsタブとProfilerタブです。これらを常に使用するので、ドラッグすることもできます。

ここでReact要素ツリーの読み込みと表示されます。これは高速であるはずです。もう一度試してみましょう。

実際、ブラウザを閉じて再度開いた後にのみ機能しました。以前のエラーメッセージと同じエラーメッセージが表示された場合は、今すぐ同じことをしてください。

しかし、今、すべてが機能しています。これらの開発者ツールを見てみましょう。特に開発者ツールのComponents部分を見てみましょう。

実際には、Profilerという別の部分がありますが、後でまた戻ってきます。

### Componentsタブの活用

Componentsは、名前が示すように、コンポーネントツリーを表示するためです。今、コンポーネントは1つしかありません。appコンポーネントのみです。

ここですべての選択されたコンポーネントが受け取っているすべてのpropsが表示されます。この場合、もちろん、propsは受け取っていないので、何もありません。

しかし、ここに興味深い部分があります。ここに、私たちが使用した各Stateフックのすべてのフックのリストがあります。useStateで状態を作成したことを覚えています。これらのuse関数はフックです。したがって、フックのリストにあります。

### 状態の操作と実験

いずれにせよ、興味深いのは、これらの値をここで操作して、それらを実験できることです。

たとえば、boolean値がある場合、チェックボックスが表示され、それを切り替えることができます。これにより、値も切り替わります。trueからfalseへ。UIでできることと同じことをしています。それは非常に役立ちます。

そして、CSSでできることと少し似ています。これはCSSに触発されています。

ここで、この数値を変更することもできます。1から3に直接移動できます。または、もちろん、UIから通常アクセスできない値で試すこともできます。

これらのボタンをクリックしても、状態を10に設定することはできません。しかし、何らかの理由でUIが10でどのように見えるかを確認する必要があるかもしれません。開発者ツールに来て、それをこれに設定できます。

したがって、これは単なる小さなデモ例です。ここでは重要ではありません。しかし、より大きく、大規模なアプリケーションでは、これは時々必要になるかもしれません。

開発者ツールをこの種のことに使用できることを覚えておくことは非常に重要です。それは1つのことです。

### コンポーネントツリーの表示

前述したように、コンポーネントツリー全体をここに表示できることは非常に役立ちます。プロジェクトに多くのファイルがあり、アプリに数十または数百のコンポーネントがある場合、それはすぐに手に負えなくなり、どのコンポーネントがどこにあるかを見失う可能性があります。

コンポーネントツリーは非常に便利になります。それを手動で描画する代わりに、ここに来て、それがここにあります。

そして今のところ、開発者ツールについて話すことはすべてです。非常に便利です。それらをインストールすることを確認してください。そして、将来の講義で必ずまた戻ってきます。

## 現在の状態に基づいて状態を更新する

状態変数をその状態の現在の値に基づいて更新することは非常に一般的です。そして、それを行う最善の方法を学びましょう。

実際、私たちは常に状態を現在の状態に基づいて更新しています。ここで、たとえば、setStepで、現在のステップを取り、そして、1を引きます。ここでも同じです。現在のisOpen状態を取り、それを切り替えています。

これは私が現在の状態に基づいて状態を更新することによって意味することです。

### 現在のアプローチの問題点

私たちが今やっている方法はうまくいっています。私たちのアプリはうまくいっていますが、数ヶ月後にこのアプリに戻ってきて、何かを変更したいと想像してみましょう。
