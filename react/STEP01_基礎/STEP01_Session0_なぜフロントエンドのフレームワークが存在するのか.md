...
さて、このセッションでその疑問に答えていきましょう。過去にどのようにウェブサイトが構築され、どのように新しい方法へと移行し、フロントエンドフレームワークの台頭へとつながっていったのかを振り返りながら、基礎から整理### 🔄 React の主な目標：UI をデータと同期させ続けること

React の主な目標は、常に UI をデータと同期させることであることを思い出してください。

そして今後は、実際にそのデータを**ステート**と呼ぶことにしましょう。

例えば、Airbnb のアパートの配列などです。

初期ステートに基づいて、React は JSX を使って私たちが書いたコンポーネントを使用してユーザーインターフェイスをレンダリングします。

次に、ボタンのクリックなどのイベントに基づいて、ステートが変わる可能性があります。

例えば、アパートに関するより多くのデータが API から読み込まれる場合などです。

そしてここが興味深い部分です。

ステートが変更されるたびに、私たちはアプリ内のステートを手動で更新し、React は自動的にユーザーインターフェイスを再レンダリングして、その最新のステートを反映します。

言い換えると、React はユーザーインターフェイスを再レンダリングすることで、ステートの変更に反応（react）するのです。

そして実際、これが React が最初に React と呼ばれている理由なのです。## 📜 ウェブ開発の歴史：サーバーサイドレンダリングの時代
...
実際、ここに挙げたリストでさえ、すべてではありません。

**複雑な相互依存関係**

ご存知のように、これらのデータはすべて、ユーザーインターフェースと常に同期している必要があり、また他のデータとも同期している必要があります。なぜなら、これらはすべて相互に関連しているからです。
...
そのため、これらのデータ同士はさらに相互に関連しており、本当に混乱する可能性があります。
...
これらのフレームワークは、この本当に困難な問題を解決してくれるので、開発者はデータ自体とユーザーインターフェース自体の構築だけに集中できます。

さて、異なるフレームワークがこれを実現するアプローチは異なりますが、それらはすべて UI とデータを時間の経過とともに同期させるという点で共通しています。
...
> React とは、**非常に人気が高く、宣言的で、コンポーネントベースで、ステート駆動な、ユーザーインターフェイス構築のための JavaScript ライブラリ（Facebook によって作成）** である。

この拡張された定義の方が実際にずっと有用です。なぜなら、この定義を分解して、React が実際に何なのかを段階的に学んでいくことができるからです。
...
実際、React が基本的に行うことは、コンポーネントを受け取り、それらをウェブページ上に描画することです。それだけです。
つまり、React では複数のコンポーネントを構築し、それらのコンポーネントをレゴのピースのように組み合わせることで、複雑な UI を構築します。
例えば、Airbnb のような複雑なアプリケーションを作成する場合を考えてみましょう。
ここには、先ほど見たのと全く同じコンポーネントがありますが、それらが組み合わされて複雑な UI を形成しています。ナビゲーションバー、検索バー、結果パネル、そして地図です。
さらに、コンポーネントで行うもう一つのことは、それらを再利用することです。
例えば、ここの結果パネルには、非常に似た見た目の複数のリストがあります。そこで、リスティングコンポーネントを作成して、それをここで 3 回再利用し、データだけを少し調整するのです。
複数のコンポーネントを組み合わせることで複雑な UI を構築するため、各コンポーネントは自分がどのように見えるべきかについてのすべての情報を持っている必要があります。
...
要約すると、React は実際に 2 つのことを得意としています。

第一に、現在のステートに基づいて、コンポーネントをウェブページ上にユーザーインターフェイスとしてレンダリングすること。

第二に、React はコンポーネントの一つのステートが変更されたときに、再レンダリングによって、つまり反応することによって、ユーザーインターフェイスをステートと同期させ続けることを得意としています。

そして後で見るように、React はバーチャル DOM、Fiber ツリー、一方向データフロー、そしてこのフェーズで学習する多くの他の技術を使用してこれらすべてを行います。
