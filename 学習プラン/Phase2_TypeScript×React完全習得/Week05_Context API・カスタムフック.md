...
**📌 重要**: Week 5 は React の状態管理を完全に理解し、実践的なアプリケーション開発の基盤を築く重要な期間です。Context API とカスタムフックの習得により、後の高度な状態管理ライブラリ学習がスムーズに進みます。