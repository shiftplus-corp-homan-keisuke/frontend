...
function isUserApiResponse(value: unknown): value is UserApiResponse {
  ...
  return (
    ...
    obj.profile !== null &&
    isNonEmptyString(obj.profile.firstName) &&
    isNonEmptyString(obj.profile.lastName)
  );
}

class ApiClient {
  static async fetchUser(userId: number): Promise<UserApiResponse> {
...
function validateContactForm(formData: unknown): ContactFormData | null {
  ...
  try {
    ...
    assertIsValidEmail(data.email);
    assertIsString(data.message);
...
function isBusinessLogicError(error: unknown): error is BusinessLogicError {
...
function handleError(error: unknown): string {
  if (isNetworkError(error)) {
    ...
    return `ビジネスロジックエラー (${error.code}): ${error.message}`;
  } else if (error instanceof Error) {
    return `一般エラー: ${error.message}`;
  } else {
    return `未知のエラー: ${String(error)}`;
  }
}
```
...
2. **ユーザー定義型ガード**
   - 複雑な条件や再利用可能な型チェック
   - `value is Type`構文で独自の型チェック関数を作成

3. **アサーション関数**
   - `asserts value is Type`でエラー時の型保証
   - 実行時型チェックとエラーハンドリングの統合

### 実践での重要ポイント

1. **段階的な型ガード**: 複雑な型は小さな型ガードを組み合わせて構築
2. **エラーハンドリング**: アサーション関数で適切な情報提供
3. **保守性**: 型ガード関数は再利用可能で理解しやすい形で実装
4. **パフォーマンス**: 型ガードは実行時コストがあることを意識

### 次のステップへの準備

STEP04で学習した型ガードの概念は、以下の高度なTypeScript機能の基礎となります：

- **条件付き型（Conditional Types）**: 型レベルでの条件分岐
- **マップ型（Mapped Types）**: 既存の型から新しい型を生成
- **テンプレートリテラル型**: 文字列リテラル型の高度な操作

## 🔗 関連リソース

- [TypeScript公式ドキュメント - Type Guards](https://www.typescriptlang.org/docs/handbook/2/narrowing.html)
- [TypeScript公式ドキュメント - Assertion Functions](https://www.typescriptlang.org/docs/handbook/release-notes/typescript-3-7.html#assertion-functions)

---

**🎯 総復習完了**

STEP04「型ガード完全習得」の理論学習内容を体系的に整理しました。基本的な型ガードから高度なアサーション関数まで、実践的な活用方法を含めて学習できる内容となっています。