...
function TaskDetailModal({ taskId, isOpen, onClose }: TaskDetailModalProps): JSX.Element {
    ...
    <Modal isOpen={isOpen} onClose={onClose} size="large">
      <div className="task-detail">
        <TaskDetailHeader
          ...
          onSave={(data) => updateTaskMutation.mutateAsync({ id: taskId, data })}
        ...
        <div className="task-comments">
          ...
          <CommentInput
            value={comment}
            onChange={setComment}
            onSubmit={() => addCommentMutation.mutateAsync({ taskId, content: comment })}
          />
        </div>
      </div>
    </Modal>
...
function FileUpload({ onUpload, accept, multiple = false, maxSize = 10 * 1024 * 1024 }: FileUploadProps): JSX.Element {
  ...
  return (
    ...
    >
      ...
      <div className="upload-content">
        ...
        <p className="upload-hint">最大サイズ: {Math.round(maxSize / 1024 / 1024)}MB</p>
      </div>
    </div>
  );
}
...
test.describe('TaskFlow App', () => {
  test('should complete task workflow', async ({ page }) => {
    ...
    // ステータスが更新されたことを確認
    await expect(
      page.locator('[data-testid="column-in_progress"] [data-testid="task-item"]')
...
```typescript
// Week 10で学習するプロジェクト完成・デプロイの基礎概念
...
**📌 重要**: Week 9 は学習した全ての技術を統合し、実用的なアプリケーションを完成させる重要な期間です。プロダクション品質を目指して丁寧に実装してください。