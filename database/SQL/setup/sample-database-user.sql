...
-- サンプルデータベース作成スクリプト（一般ユーザー権限用）
-- ============================================================================

-- ============================================================================
-- 【重要】実行前の確認事項
-- ============================================================================
-- このスクリプトは一般ユーザー権限で実行できます。
-- 以下の前提条件があります：
-- - データベース 'learning_db' が既に作成されていること
-- - 実行ユーザーが learning_db に対して以下の権限を持っていること：
--   * SELECT, INSERT, UPDATE, DELETE (データ操作権限)
--   * CREATE, DROP, INDEX, ALTER (テーブル操作権限)
-- 
-- 実行方法：
-- MySQL: mysql -u learning_user -p learning_db < sample-database-user.sql
-- PostgreSQL: psql -U learning_user -d learning_db -f sample-database-user.sql
-- 
-- 注意：データベース作成権限は不要です
-- ============================================================================

-- データベースが存在することを確認（情報表示のみ）
SELECT 'learning_db データベースを使用します' as message;
...
DROP PROCEDURE IF EXISTS monthly_sales_report;
...
SELECT 'サンプルデータベースの作成が完了しました！（一般ユーザー権限版）' as message;
SELECT 'Level 1-3の学習に必要なテーブル、データ、ビュー、プロシージャが準備されています。' as info;
SELECT 'データベース作成権限は不要で、既存のlearning_dbデータベース内で実行されました。' as note;