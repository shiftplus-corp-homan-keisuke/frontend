...
| ------------ | ------------------------------ | -------------------------- | -------------- | ---------------- |
| **0-5分**    | 前回復習・最終課題説明         | 復習確認・課題説明         | 理解・質問     | 実装計画         |
| **5-20分**   | アサーション関数理論と実装     | 実演・解説                 | 理解・実践     | 基本実装         |
| **20-35分**  | フォーム処理システム完成       | 個別サポート・デバッグ支援 | 開発・完成     | 完成プロジェクト |
| **35-40分**  | 成果発表・Step04総復習         | 評価・フィードバック       | 発表・振り返り | 学習成果         |
...
アサーション関数は、型ガードの結果が`false`の場合にエラーを投げることで、その後のコードで型が保証されることをTypeScriptに伝える機能です。
...
アサーション関数は、条件が満たされない場合にエラーを投げることで、その後のコードで型が保証されることをTypeScriptに伝える機能です。これにより、以下のメリットがあります：
...
class InvalidFormatError extends ValidationError {
  constructor(field: string, expectedFormat: string) {
    super(`${field}の形式が正しくありません。期待される形式: ${expectedFormat}`, field);
    this.name = "InvalidFormatError";
  }
}

// カスタムエラーを使用するアサーション関数
function assertIsValidEmail(value: unknown, fieldName: string = "メールアドレス"): asserts value is string {
  if (typeof value !== "string") {
    throw new RequiredFieldError(fieldName);
  }
  
  if (!isValidEmail(value)) {
    throw new InvalidFormatError(fieldName, "example@domain.com");
  }
}

function assertIsValidAge(value: unknown, fieldName: string = "年齢"): asserts value is number {
  if (typeof value !== "number") {
    throw new RequiredFieldError(fieldName);
  }
  
  if (value < 0 || value > 150) {
    throw new ValidationError(`${fieldName}は0-150の範囲で入力してください`, fieldName);
  }
}
```
...
```typescript
// 条件付きアサーション関数
function assertIsDefined<T>(value: T | null | undefined, fieldName?: string): asserts value is T {
  if (value === null || value === undefined) {
    const message = fieldName ? `${fieldName}は必須です` : "値はnullまたはundefinedにできません";
    throw new RequiredFieldError(fieldName || "値");
  }
}

function assertIsOneOf<T>(value: unknown, allowedValues: T[], fieldName?: string): asserts value is T {
...
### Section 3: 型安全なフォーム処理システム完成

> 📚 **関連資料**: [実践コード例 - フォーム処理システム完全版](./Step04_補足_実践コード例.md#フォーム処理システム完全版) | [トラブルシューティング - デバッグガイド](./Step04_補足_トラブルシューティング.md#デバッグのコツ)

#### 🔧 最終課題: 型安全なフォーム処理システム

**💡 実装する機能**

Step01-04で学んだ知識を統合して、型安全なフォーム処理システムを実装します。
...
- Step04: 型ガード、ユーザー定義型ガード、アサーション関数
...
function assertIsValidPassword(value: unknown): asserts value is string {
  ...
  if (value.length < 8) {
    throw new ValidationError("パスワードは8文字以上で入力してください", "password");
  }
  ...
  if (!hasUpperCase || !hasLowerCase || !hasNumber) {
    throw new ValidationError("パスワードは大文字・小文字・数字を含む必要があります", "password");
  }
}
...
function assertIsValidMessage(value: unknown): asserts value is string {
  ...
  if (value.length > 1000) {
    throw new ValidationError("メッセージは1000文字以内で入力してください", "message");
  }
}
...
class FormProcessor {
  ...
  // 汎用フォーム処理
  static processForm(formData: unknown, formType: "registration" | "contact"): FormProcessingResult<FormData> {
    switch (formType) {
      case "registration":
...
function runTests(): void {
  ...
  if (result2.data) {
    console.log(`お問い合わせ者: ${result2.data.name} (${result2.data.contactMethod})`);
  }

  // 無効なユーザー登録フォーム
  console.log("\n--- 無効なユーザー登録フォーム ---");
  const result3 = FormProcessor.processUserRegistration(invalidUserRegistration);
...
### 練習問題 3.1: 基本的なアサーション関数 🔰

以下の要件を満たすアサーション関数を実装してください。

```typescript
// 要件1: 値がnullまたはundefinedでないことをアサートする関数
function assertIsDefined<T>(value: T | null | undefined): asserts value is T {
  /* ここを実装 */
}

// 要件2: 配列が空でないことをアサートする関数
function assertIsNonEmptyArray<T>(array: T[]): asserts array is [T, ...T[]] {
  /* ここを実装 */
}

// 要件3: 上記のアサーション関数を使用する関数
function getFirstElement<T>(items: (T | null | undefined)[]): T {
  // 配列が空でないことをアサート
  // 最初の要素がnullまたはundefinedでないことをアサート
  // 最初の要素を返す
  /* ここを実装 */
}
```

### 練習問題 3.2: カスタムエラーとアサーション関数 🔰
...
// 要件: Product型のアサーション関数を実装
function assertIsValidProduct(value: unknown): asserts value is Product {
  // id: 空でない文字列
  // name: 空でない文字列（100文字以内）
  // price: 正の数値
  // category: 空でない文字列
  // エラー時はProductValidationErrorを投げる
  /* ここを実装 */
}
```
...
function assertIsValidProduct(value: unknown): asserts value is Product {
  if (typeof value !== "object" || value === null) {
    throw new ProductValidationError("商品データはオブジェクトである必要があります");
  }

  const obj = value as any;

  if (typeof obj.id !== "string" || obj.id.trim().length === 0) {
    throw new ProductValidationError("商品IDは空でない文字列である必要があります", obj.id);
  }

  if (typeof obj.name !== "string" || obj.name.trim().length === 0) {
    throw new ProductValidationError("商品名は空でない文字列である必要があります", obj.id);
  }

  if (obj.name.length > 100) {
    throw new ProductValidationError("商品名は100文字以内である必要があります", obj.id);
  }

  if (typeof obj.price !== "number" || obj.price <= 0) {
    throw new ProductValidationError("価格は正の数値である必要があります", obj.id);
  }

  if (typeof obj.category !== "string" || obj.category.trim().length === 0) {
    throw new ProductValidationError("カテゴリは空でない文字列である必要があります", obj.id);
  }
}
```
...
A: try-catch文で適切にエラーハンドリングを行い、ユーザーに分かりやすいエラーメッセージを表示することが重要です。また、カスタムエラークラスを使用することで、エラーの種類に応じた処理を行えます。
...
#### 技術習得度（60%）
- [ ] **基本型ガード**: typeof、instanceof、in演算子の適切な使用
